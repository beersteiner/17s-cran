...
parser.add_argument('-e', '--epochs', metavar='E', type=int, default=1,
                    help='Number of epochs to train attack model', required=False)
...
import numpy as np
if args.analyze: from atk_features import analyze
from atk_features import structArray, ewFeat
...
for f in args.files:
    ...
    checkpoint = ModelCheckpoint('./atkmodels/atk_'+os.path.split(f.name)[1][:4]+'_e'+str(N_EPOCH)+'.hdf5', monitor='val_acc', verbose=True,
            save_best_only=True, save_weights_only=False, mode='auto', period=1)
    csv_logger = CSVLogger('./atkmodels/atk_'+os.path.split(f.name)[1][:4]+'_e'+str(N_EPOCH)+'_trng.log')
    ...
    history = model.fit(
            ...
            verbose=1, 
            validation_data=(Xtst[feat].view(np.float64).reshape(Xtst.shape + (len(feat),)), Ytst))
    score = model.evaluate(
            Xtst[feat].view(np.float64).reshape(Xtst.shape + (len(feat),)),
            Ytst,
            verbose=0)
